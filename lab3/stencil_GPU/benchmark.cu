...
int main(int argc, char **argv)
{
    ...
    ABORT_IF_ERROR(read_file(a0, argv[6], &info, host_buffer));
    ...
    if (argc == 8)
    {
        ...
        ans1 = (ans0 == a0) ? a1 : a0;
        ABORT_IF_ERROR(read_file(ans1, argv[7], &info, host_buffer))
    ...
    gflops = 1e-9 * info.global_size_x * info.global_size_y * info.global_size_z * nt * (type * 2 - 1) / run_time;
...
int read_file(ptr_t grid, const char *file_name, const dist_grid_info_t *info, ptr_t buffer)
{
    ...
    count = fread(buffer, sizeof(data_t), nx * ny * nz, file);
    fclose(file);
    if (count != nx * ny * nz)
    {
