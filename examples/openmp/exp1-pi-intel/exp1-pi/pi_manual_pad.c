...
int main(int argc, char* argv[]) {
    ...
    // set the number of threads
        ...
        double *temp = _mm_malloc(sizeof(double)*num_threads*8,64);
    #endif
    ...
    for (i = 0; i < num_steps; i++) {
    	int posID = 8*omp_get_thread_num();
        x = (i + .5) * step;
        //sum += 4.0 / (1.+ x * x);
       	temp[posID] = temp[posID] + 4.0 / (1.+ x * x);
    }

    for(i=0;i<num_threads;i++) sum+=temp[i*8];
