...
static inline void do_block_divide_simd(int lda, int M, int N, int K, float *A, float *B, float *C)
{
    ...
    for (int i = 0; i < M; i += 4)
    {
        ...
        for (int j = 0; j < N; j += 4)
        {
            ...
            for (int k = 0; k < K; k += 4)
            {
                __m128 a_0 = _mm_load_ps(&A(4 * k, i / 4)); // a_0k, a_1k, a_2k, a_3k
                __m128 a_1 = _mm_load_ps(&A(4 * k + 4, i / 4));
                __m128 a_2 = _mm_load_ps(&A(4 * k + 8, i / 4));
                __m128 a_3 = _mm_load_ps(&A(4 * k + 12, i / 4));
...
{
    // copy A[i + k * lda] to ABC[4 * k + i / 4 * BLOCKSIZE]
    for (int k = 0; k < K; k++)
    {
        for (int i = 0; i < M; i += 4)
        {
            float *p_ABC = ABC + i / 4 * BLOCK_SIZE + 4 * k;
            float *p_A = A + i + k * lda;
            memcpy(p_ABC, p_A, 4 * sizeof(float));
        }
    }
