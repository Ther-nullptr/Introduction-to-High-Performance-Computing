...
int main(int argc, char **argv)
{
    ...
    for (int isize = 0; isize < sizeof(test_sizes) / sizeof(test_sizes[0]); ++isize)
    {
        ...
        memset(C, 0, n_padding * n_padding * sizeof(float));
        for (int i = 0; i < n_padding; ++i)
        {
            for (int j = 0; j < n_padding; ++j)
            {
        ...
        reference_sgemm(n_padding, -1., A, B, C);
        /* A := |A|, B := |B|, C := |C| */
        absolute_value(A, n_padding * n_padding);
        absolute_value(B, n_padding * n_padding);
        absolute_value(C, n_padding * n_padding);

        /* C := |C| - 3 * e_mach * n * |A| * |B|, computed with reference_sgemm */
        reference_sgemm(n_padding, -3. * FLT_EPSILON * n_padding, A, B, C);
        ...
        for (int i = 0; i < n_padding; ++i)
        {
            for (int j = 0; j < n_padding; ++j)
            {
                if (i < n && j < n)
                {
                    printf("(%f, %f)", C[i + j * n_padding], initial);
                }
            }
        }
    }
